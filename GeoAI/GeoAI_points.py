...
def geoguessr_score(pred, target, max_score=5000):
    c = 40000 / 2  # Half Earth's circumference in km
    score = max_score * torch.exp(-10*haversine_distance(pred, target, reduction=None)/c)
    return score

def geoguessr_loss(pred, target):
    return -geoguessr_score(pred, target).mean()

seed = random.randint(0, 10000)
seed = 8661 # Fixed seed for reproducibility, comment out to use random seed
